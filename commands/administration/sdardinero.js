...
module.exports = {
    name: 'sdardinero',
    description: 'Suma o resta dinero de la columna cash en la tabla player.',
    async execute(message, args, pool) {
        ...
        if (!userNameOrId || isNaN(amount)) {
            const embed = new EmbedBuilder()
                .setColor('#FF0000')
                .setAuthor({
                    name: `❌ Error SDARDINERO ${message.member.displayName}`,
                    iconURL: message.author.displayAvatarURL()
                })
                .setDescription('Modo de uso: !sdardinero @usuario|nombre_apellido cantidad.')
                .setTimestamp()
        ...
        try {
            ...
            if (usuario.connected !== 0) {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`🧍‍♂️· **${usuario.name} (${usuario.id}RP)** debe estar **(OFF)** para recibir **${amount > 0 ? '+$' : '-$'}${Math.abs(amount)}**.`)
                    .setTimestamp()
                    .setFooter({ text: `${message.guild.name}`, iconURL: message.guild.iconURL() });
                await message.channel.send({ embeds: [embed] });
                await message.delete().catch(console.error);
            ...
            const nuevoCash = usuario.cash + amount;
            ...
            const embed = new EmbedBuilder()
                .setColor('#00FF00')
                .setAuthor({
                    name: `💵 Comando SDARDINERO ${message.member.displayName}`,
                    iconURL: message.author.displayAvatarURL()
                })
                .setDescription(`🏦·**${usuario.name} (${usuario.id}RP)** ha recibido **🤑 ${amount > 0 ? '+' : '-'}${Math.abs(amount)}**.\n\n💰**Saldo actual:** ${nuevoCash}`)
                .setTimestamp()
                .setFooter({ text: `${message.guild.name}`, iconURL: message.guild.iconURL() });
            await message.channel.send({ embeds: [embed] });
            await message.delete().catch(console.error);
