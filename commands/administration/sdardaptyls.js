...
module.exports = {
    name: 'sdardaptyls', // Comando para dar coins
    description: 'Suma coins a la columna coins en la tabla player.',
    async execute(message, args, pool) {
        ...
        if (!userNameOrId || isNaN(amount)) {
            const embed = new EmbedBuilder()
                .setColor('#FF0000')
                .setAuthor({
                    name: `❌ Error SDARDAPTYLS ${message.member.displayName}`,
                    iconURL: message.author.displayAvatarURL()
                })
                .setDescription('Modo de uso: !sdardaptyls @usuario|nombre_apellido cantidad.')
                .setTimestamp()
        ...
        try {
            ...
            if (usuario.connected !== 0) {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(`🧍‍♂️· **${usuario.name} (${usuario.id}RP)** debe estar **(OFF)** para recibir **${amount} coins**.`)
                    .setTimestamp()
                    .setFooter({ text: `${message.guild.name}`, iconURL: message.guild.iconURL() });
                await message.channel.send({ embeds: [embed] });
                await message.delete().catch(console.error);
            ...
            const nuevoCoins = usuario.coins + amount;  // Sumar coins en lugar de restar

            // Actualizar la cantidad de coins
            await connection.query('UPDATE player SET coins = ? WHERE id = ?', [nuevoCoins, usuario.id]);

            const embed = new EmbedBuilder()
                .setColor('#00FF00')  // Cambié el color a verde, ya que se suman coins
                .setAuthor({
                    name: `💰 Comando SDARDAPTYLS ${message.member.displayName}`,
                    iconURL: message.author.displayAvatarURL()
                })
                .setDescription(`🏦·**${usuario.name} (${usuario.id}RP)** ha recibido **🤑 ${amount} coins**.\n\n💰**Saldo actual:** ${nuevoCoins} coins`)
                .setTimestamp()
                .setFooter({ text: `${message.guild.name}`, iconURL: message.guild.iconURL() });
            await message.channel.send({ embeds: [embed] });
            await message.delete().catch(console.error);
